...
public class DB {
	...
	public DB() {
	 ...
	 public boolean AddPlayer(Player player ) throws SQLException{
		 try {
			 String sql = "insert into Players(PlayerID,Name,Age,Position) VALUES (" + player.getPlayerID() + ",'"+player.getName()+ "',"+ player.getAge() +",'"+ player.getPosition() +"')";
	 ...
	 public ArrayList<Player> getPlayer()
	 {
		 ...
		 try {
			 ...
			 Player player;
			 while (rs.next()) {
				 player = new Player(rs.getInt("PlayerID"),rs.getString("Name"),rs.getString("Position"),rs.getFloat("Age"));
	 ...
	 public ArrayList<Game> getGames()
	 {
		 ...
		 try {
			 ...
			 Game game;
			 while (rs.next()) {
				 game = new Game(rs.getInt("GameID"),rs.getString("Team_opponent"),rs.getInt("GoalHome"),rs.getInt("GoalGuest"),rs.getDate("Date"));
				 allGame.add(game);
			 }
	 ...
	 public boolean AddGame(Game game) throws SQLException{
		 try {
			 String sql = "insert into Games(GameID,Team_opponent,GoalHome,GoalGuest,Date) VALUES(" + game.getGameID() + ",'" + game.getTeam_Opponent() +"'," + game.getGoalHome() + "," + game.getGoalGuest() + ",'" + game.getDate() +"')";
			 System.out.print(sql);
			 if (stmt.executeUpdate(sql) == 1) 	
				 return true;
			 else 
